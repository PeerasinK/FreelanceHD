    ...
    public class Sales
    {
        ...
        public DataSet GetSalesOrderList()
        {
            ...
            else
            {
                ...
                if (_order_recv_date != 0)
                {
                    strSQL.Append(" AND     S.ORDER_RECV_DATE            = @ORDER_RECV_DATE                             ");
                    db.DbPsetInt("@ORDER_RECV_DATE", _order_recv_date);
                }
            }
        ...
        public DataSet GetOrdReturnCancelList()
        {
            ...
            strSQL.Append(" '' AS ORDER_NO                ,SA.ITEM_NO             ,I.ITEM_DESC ,                ");    // IMV3
            strSQL.Append(" SA.RETURN_QTY                ,SA.RETURN_DATE             ,SA.RETURN_WHS           ,W.WHS_DESC                                           ");
            strSQL.Append(" ,R.TO_LOCATION            ");  // IMV3
            if (_db_type == "ORACLE")
            {
            ...
            else
            {
                ...
                //strSQL.Append(" LEFT JOIN SALES_ORDER_DETAIL SAD ON SA.ORDER_NO=SAD.ORDER_NO        ");   // IMV3
                //strSQL.Append(" AND SA.ORDER_LINE_NO       = SAD.ORDER_LINE_NO                      ");   // IMV3
                // IMV3 START
                strSQL.Append(" LEFT JOIN RCV_DISB_TRANS R  ON SA.RETURN_NO=R.SLIP_NO AND R.REQ_TYPE='26' AND SA.RETURN_DATE = R.TRANS_DATE ");
                // IMV3 END
        ...
        /// 返品取消
        ///
        /// 使用画面：OrdReturnCancel   IMV3
        /// </summary>
        public DataSet GetOrdReturnCancelListIM()
        {
            ComDB db = new ComDB(_db_type);
            StringBuilder strSQL = new StringBuilder();
            DataSet ds = new DataSet();
            ...
            // sql
            strSQL.Append(" SELECT                                                                                                               ");
            strSQL.Append(" TR.XFER_NO             ,(TR.FROM_VC_CD+'-'+TR.FROM_COST_CENTER) AS CUST_CD  ,C.CUST_NAME ,         TR.SLIP_DATE    , ");
            strSQL.Append(" TR.ITEM_NO             ,TR.ITEM_DESC                          , TR.ORDER_NO ,TR.INST_QTY RETURN_QTY                , ");
            strSQL.Append(" TR.TO_WHS_CD RETURN_WHS, TR.XFER_TEXT   ,   TR.TO_LOCATION    , TR.SLIP_DATE RETURN_DATE,     ");
            ...
            strSQL.Append(" LEFT JOIN CUSTOMER_MASTER C ON TR.FROM_VC_CD=C.CUST_CD                 ");
            strSQL.Append(" AND TR.FROM_COST_CENTER       = C.CUST_SUB_CD                            ");
            strSQL.Append(" WHERE (TR.XFER_STATUS  = 20                                                         ");
            strSQL.Append(" OR TR.XFER_STATUS     = 0                                                           "); // IMV3
            strSQL.Append(" OR TR.XFER_STATUS     = 50   )                                                      ");
            strSQL.Append(" AND TR.REQ_TYPE          = '26'                                                     ");


            db.DbParametersClear();
            if (_return_no != "")
            {
                strSQL.Append("   AND TR.XFER_NO      = @RETURN_NO");
                db.DbPsetString("@RETURN_NO", _return_no);
            }
            if (_order_no != "")
            {
                strSQL.Append("   AND TR.ORDER_NO      = @ORDER_NO");
                db.DbPsetString("@ORDER_NO", _order_no);
            }
            if (_cust_cd != "")
            {
                strSQL.Append("   AND TR.FROM_VC_CD      = @CUST_CD");
                db.DbPsetString("@CUST_CD", _cust_cd);
            }
            if (_cust_sub_cd != "")
            {
                strSQL.Append("   AND TR.FROM_COST_CENTER      = @CUST_SUB_CD");
                db.DbPsetString("@CUST_SUB_CD", _cust_sub_cd);
            }
            if (_item_no != "")
            {
            ...
            if (_return_date1 != 0)
            {
                strSQL.Append("   AND TR.SLIP_DATE     >= @RETURN_DATE1 ");
                db.DbPsetInt("@RETURN_DATE1", _return_date1);

            }
            if (_return_date2 != 0)
            {
                strSQL.Append(" AND TR.SLIP_DATE     <= @RETURN_DATE2 ");
                db.DbPsetInt("@RETURN_DATE2", _return_date2);
            }
            try
            {
        ...
        /// <summary>
        /// 顧客返品取消 OrdReturnCancelの実行 IMV3
        ///
        /// <para>使用画面：OrdReturnCancel</para>
        /// </summary>
        public int OrdReturnCancelIM()
        {
            ComDB db = new ComDB(_db_type);
            int rtn = 0;
            ...
            db.DbParametersClear();
            db.DbPsetInt("RET", rtn, ComConst.DB_RTN);
            db.DbPsetString("@I_XFER_NO", _return_no, ComConst.DB_IN);

            db.DbPsetString("@I_CHG_PGM", _chg_pgm, ComConst.DB_IN);
            db.DbPsetString("@I_CHG_USERID", _chg_user_id, ComConst.DB_IN);
            db.DbPsetInt("@I_LANG", _lang, ComConst.DB_IN);
            db.DbPsetInt("@I_UPDATE_CNTR", 0, ComConst.DB_IN);
            db.DbPsetInt("@O_ERRCODE", _errcode, ComConst.DB_OUT);
            db.DbPsetString("@O_MSG", _errmsg, ComConst.DB_OUT);
            db.DbPsetInt("@O_SQLCODE", _sqlcode, ComConst.DB_OUT);
            db.DbPsetString("@O_SQLMSG", _dbmsg, ComConst.DB_OUT);
        ...
        /// 使用画面：OrdReturnAct
        /// </summary>
        public DataSet GetOrdReturnActList()
        {
            ComDB db = new ComDB(_db_type);
            StringBuilder strSQL = new StringBuilder();
            DataSet ds = new DataSet();
            ...
            //--- sql
            strSQL.Append("SELECT T.SLIP_NO + '-' + LTRIM(STR(T.SLIP_LINE_NO)) SLIP_NO, T.ORDER_NO,                  ");
            strSQL.Append(" T.XFER_NO,T.TRANS_DATE REC_DATE,T.ITEM_NO,T.ITEM_DESC,C.CUST_NAME,  ");
            ...
            strSQL.Append(" INNER JOIN TRANSFER_TRANS T ON T.XFER_NO = HT.XFER_NO AND T.REQ_TYPE = 26 ");
            strSQL.Append(" LEFT JOIN SYSTEM_PARAMETER S ON S.KEY01 = 'ITEM_UMSR' AND S.KEY02 = T.ITEM_UMSR ");
            strSQL.Append(" LEFT JOIN CUSTOMER_MASTER C ON T.FROM_VC_CD=C.CUST_CD                 ");
            strSQL.Append(" AND T.FROM_COST_CENTER       = C.CUST_SUB_CD                            ");
            ...
            strSQL.Append(" WHERE ");
            if(_reissue_flag == 0)
                strSQL.Append(" HT.HT_SEND_STATUS < 3 ");
            else
                strSQL.Append(" HT.HT_SEND_STATUS = 3 ");
            strSQL.Append(" AND HT.HT_ACT_QTY > 0 ");
            ...
            if (_return_date1 != 0)
            {
                ...
                db.DbPsetInt("@TRANS_DATE", _return_date1);
            }
            if (_return_date2 != 0)
            {
                strSQL.Append(" AND T.TRANS_DATE   <=       @TRANS_DATE                            ");
                db.DbPsetInt("@TRANS_DATE", _return_date2);
            }
        ...
        /// 顧客返品伝票発行(IMV3)
        /// 
        /// <para>使用画面：[OrdReturnList]</para>
        /// </summary>
        public DataSet GetOrdReturnList()
        {
            ComDB db = new ComDB(_db_type);
            StringBuilder strSQL = new StringBuilder();
            DataSet ds = new DataSet();
            ...
            //--- sql
            strSQL.Append(" SELECT                                                                                      ");
            strSQL.Append(" T.SLIP_NO,          T.XFER_SCH_DATE,                T.TO_WHS_CD FROM_WHS_CD,                          ");
            strSQL.Append(" WHS1.WHS_DESC FROM_WHS_DESC,                                                                ");
            strSQL.Append(" T.SLIP_LINE_NO,     T.ITEM_NO,      T.ITEM_DESC,    T.TO_LOCATION FROM_LOCATION,                        ");
            strSQL.Append(" T.TO_LOT_NO,        T.XFER_NO TO_JOC_CD,    T.INST_QTY,     S.DATA_CHAR ITEM_UMSR,   ");
            strSQL.Append(" C.CUST_NAME TO_LOCATION   ");

            db.DbParametersClear();

            strSQL.Append(" FROM TRANSFER_TRANS T                                               ");
            strSQL.Append(" LEFT OUTER JOIN VENDOR_MASTER V ON T.TO_VC_CD  =V.VENDOR_CD  ");
            strSQL.Append(" LEFT OUTER JOIN WAREHOUSE_MASTER WHS1 ON T.TO_WHS_CD=WHS1.WHS_CD  ");
            strSQL.Append(" LEFT OUTER JOIN SYSTEM_PARAMETER S ON S.KEY01 = 'ITEM_UMSR' AND T.ITEM_UMSR=S.KEY02        ");
            strSQL.Append(" LEFT JOIN CUSTOMER_MASTER C ON T.FROM_VC_CD=C.CUST_CD                 ");
            strSQL.Append(" AND T.FROM_COST_CENTER       = C.CUST_SUB_CD                            ");
            strSQL.Append(" AND S.KEY02 = T.ITEM_UMSR                                           ");
            strSQL.Append(" WHERE  T.REQ_TYPE          =       @REQ_TYPE                        ");
            if (_reissue_flag == 0)//発行の場合
            {
            ...
            if (_ship_whs != "")
            {
                ...
                db.DbPsetString("@TO_WHS_CD", _ship_whs);
            }
        ...
        public int OrdReturnAct()
        {
            ...
            db.DbBeginTrans();
            rtn = db.DbStored("SP_SALES_RETURN_ACT");
            rtn = db.DbPgetInt("RET");
            if (rtn == ComConst.SUCCEED)
            {
                db.DbCommit();
