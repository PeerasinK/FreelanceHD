...
namespace IMClass
{
    public class Ship
    {
        ...
        public DataSet GetShipInstList()
        {
            ...
            strSQL.Append(" LEFT OUTER JOIN                                                                     ");
            strSQL.Append(" ( SELECT INV.WHS_CD, INV.ITEM_NO, INV.JOC_CD,                                       ");
            strSQL.Append("      SUM(INV.INV_BAL) INV_BAL, SUM(INV.INV_BAL - INV.ALLOC_QTY) - COALESCE(PL.PLAN_QTY,0) WILL_SHIP_QTY  ");
            strSQL.Append(" FROM INVENTORY_DETAIL INV       ");
            strSQL.Append(" LEFT JOIN ( ");
            strSQL.Append(" SELECT L.ITEM_NO,L.WHS_CD,COALESCE(SUM(L.PLAN_QTY),0) PLAN_QTY  ");
            strSQL.Append(" FROM PICKING_LOT L WHERE L.PICKING_STATUS < 30 GROUP BY L.ITEM_NO,L.WHS_CD ");
            strSQL.Append(" ) PL ON PL.ITEM_NO = INV.ITEM_NO AND PL.WHS_CD = INV.WHS_CD ");
            strSQL.Append(" WHERE INV.INV_YM       = 0      ");
            strSQL.Append(" AND INV.INV_WIP_TYPE = 0        ");
            strSQL.Append(" AND INV.INV_TYPE     = 0        ");
            strSQL.Append(" GROUP BY INV.WHS_CD, INV.ITEM_NO, INV.JOC_CD,PL.PLAN_QTY ");
            strSQL.Append(" ) INV ");
            //IMV3 End
            /*
            		SELECT L.PLAN_QTY
		  FROM PICKING_LOT L
		 WHERE L.ITEM_NO = @I_ITEM_NO
		   AND L.WHS_CD  = @wWhs_Cd
		   AND L.LOCATION = @wInv_Location
		   AND L.LOT_NO   = @wInv_Lot_No
		   AND L.PICKING_STATUS < 30
            */
        ...
        public DataSet GetShipDetailList()
        {
            ...
            strSQL.Append(" ISNULL(SA.ACT_QTY,0) ALLOC_QTY_SA,    I.ITEM_UMSR,        INV.JOC_CD,                 ");   //Add 2012.05.21 Ubiq-Sai
            strSQL.Append(" ISNULL(SA.SHIP_SEQ,@SHIP_SEQ) SHIP_SEQ, ISNULL(SA.PLAN_QTY,0) SCH_SHIP_QTY          ");   //Add 2012.05.21 Ubiq-Sai
            if (_db_type == "ORACLE")
            {
            ...
            else
            {
                ...
                //strSQL.Append("         AND SA.SHIP_SEQ      = @SHIP_SEQ													");
                //strSQL.Append(" 	) SA																					");
                strSQL.Append(" 	(	SELECT 	SA.ITEM_NO, SA.WHS_CD, SA.ACT_LOCATION, SA.ACT_LOT_NO, SA.PLAN_QTY,SA.ACT_QTY,		");
                strSQL.Append(" 				SH.ORDER_NO, SH.ORDER_LINE_NO, SH.SHIP_SEQ, SH.SCH_SHIP_QTY					");
                strSQL.Append(" 		FROM PICKING_LOT SA															");
                strSQL.Append(" 		INNER JOIN SHIPPING_SCHEDULE SH														");
                strSQL.Append(" 		ON SA.PICKING_NO = SH.SHIP_NO														");
                strSQL.Append(" 		AND SA.PICKING_LINE_NO = SH.SHIP_LINE_NO											");
                strSQL.Append("	WHERE SH.ORDER_STATUS  < 90															");
                strSQL.Append("         AND SA.PICKING_STATUS  < 30     													");
                strSQL.Append("         AND SH.ORDER_NO      = @ORDER_NO													");
                strSQL.Append("         AND SH.ORDER_LINE_NO = @ORDER_LINE_NO												");
                //strSQL.Append("         AND SA.SHIP_SEQ      = @SHIP_SEQ													");
                strSQL.Append(" 	) SA																					");
                strSQL.Append(" ON INV.ITEM_NO   = SA.ITEM_NO																");
                strSQL.Append(" AND INV.WHS_CD   = SA.WHS_CD																");
                strSQL.Append(" AND INV.LOCATION = SA.ACT_LOCATION																");
                strSQL.Append(" AND INV.LOT_NO   = SA.ACT_LOT_NO																");
                //strSQL.Append(" AND INV.JOC_CD   = SA.JOC_CD																");
                strSQL.Append(" LEFT OUTER JOIN ITEM_MASTER I ON INV.ITEM_NO = I.ITEM_NO									");
        ...
        public DataSet GetShipSchList()//2008.12.03 ADD BY LIU
        {
            ...
            strSQL.Append(" SO.SHIP_TO_CD+'-'+SO.SHIP_TO_SUB_CD SHIP_NO,	SO.ADDRESS1 + SO.ADDRESS2 + SO.ADDRESS3 ADDRESS1,I.SCH_ID,  SP.DATA_CHAR SCH_NAME,   ");    // IMV3
        ...
        public DataSet GetShipInstPrtList()//2008.12.03 ADD BY LIU
        {
            ...
            strSQL.Append(" '' JOC_CD,	            SA.PLAN_QTY ALLOC_QTY,           S1.DATA_CHAR UMSR_NAME                  ");
            //strSQL.Append(" ,SUMA.SUM_ALLOC_QTY                                                                     "); //Add by ubiq -zyb 20110214
            strSQL.Append(" ,SUM.PLAN_QTY SUM_ALLOC_QTY                                                        "); //Add by ubiq -zyb 20110214
            if (_db_type == "ORACLE")
            {
                db.DbParametersClear();
            ...
            else
            {
                ...
                //strSQL.Append(" AND SS.SHIP_SEQ             = SA.SHIP_SEQ                               ");
                // IMV3
                strSQL.Append(" LEFT OUTER JOIN PICKING_LOT SA  ON SS.SHIP_NO = SA.PICKING_NO     ");
                strSQL.Append(" AND SS.SHIP_LINE_NO        = SA.PICKING_LINE_NO                          ");
                strSQL.Append(" AND SA.PICKING_STATUS      < 30  ");

                strSQL.Append(" LEFT OUTER JOIN   ");
                strSQL.Append(" (SELECT PICKING_NO,PICKING_LINE_NO,COALESCE(SUM(PLAN_QTY),0) PLAN_QTY FROM PICKING_LOT WHERE PICKING_STATUS< 30  ");
                strSQL.Append(" GROUP BY PICKING_NO,PICKING_LINE_NO) SUM  ");
                strSQL.Append(" ON SS.SHIP_NO = SUM.PICKING_NO      AND SS.SHIP_LINE_NO        = SUM.PICKING_LINE_NO       ");                      
                ...
                //strSQL.Append(" AND   SA.ALLOC_QTY          <> 0                                        ");
                strSQL.Append(" AND   SA.PLAN_QTY          <> 0                                        ");  // IMV3
        ...
        public int SalesReturn()//2008.12.04 ADD BY ZHAO
        {
            ...
            db.DbPsetString("@I_LOT_NO", _lot_no, ComConst.DB_IN);
            //db.DbPsetString("@I_LOT_NO_", _lot_no, ComConst.DB_OUT); // IMV3 DB_OUT
            db.DbPsetString("@I_LOCATION", _location, ComConst.DB_IN);
            ...
            if (rtn == ComConst.SUCCEED)
            {
                //_lot_no = db.DbPgetString("@I_LOT_NO"); // IMV3
                db.DbCommit();
            }
            else
            {
        ...
        public DataSet GetShipActList()//2008.12.04 ADD BY LIU
        {
            ...
            strSQL.Append(" SOD.ITEM_NO,            I.ITEM_DESC,             PL.ALLOC_QTY,      SS.SCH_SHIP_DATE,             ");   // IMV3
            ...
            else
            {
                ...
                strSQL.Append(" SELECT P.PICKING_NO, P.PICKING_LINE_NO, SUM(P.ACT_QTY) ALLOC_QTY ");
                strSQL.Append(" FROM PICKING_LOT  P           ");
                strSQL.Append(" WHERE P.PICKING_STATUS = 20   ");
                strSQL.Append(" GROUP BY P.PICKING_NO, P.PICKING_LINE_NO   ");
                strSQL.Append(" ) PL                          ");
                strSQL.Append(" ON PL.PICKING_NO = SS.SHIP_NO AND PL.PICKING_LINE_NO = SS.SHIP_LINE_NO ");
                // IMV3 END
                strSQL.Append(" WHERE SS.ORDER_STATUS        < 90                                        ");
                if (ship_no != "")
                {
        ...
        public DataSet GetShipActCancelList()//2008.12.05 ADD BY LIU
        {
            ...
            //strSQL.Append(" ORDER BY SOD.SHIP_WHS,SOD.ITEM_NO ,SS.SHIP_NO            ");
            strSQL.Append(" ORDER BY SOD.ACTUAL_SHIP_DATE ,SS.SHIP_NO            ");
            try
            {
                ds = db.DbDataSet(strSQL.ToString(), "TABLE");
                if (ds == null)
        ...
        public DataSet GetShipActCancelDetail()//2008.12.08 ADD BY LIU
        {
            ...
            strSQL.Append(" SA.LOT_NO,	            '' JOC_CD,              SA.LOCATION,        SA.ACT_QTY ACTUAL_SHIP_QTY,     "); // IMV3
            ...
            else
            {
                ...
                strSQL.Append("	ON SA.PICKING_NO = SH.SHIP_NO														");
                strSQL.Append("	AND SA.PICKING_LINE_NO = SH.SHIP_LINE_NO											");
                strSQL.Append(" AND SH.ORDER_STATUS       = 90  "); // 2015.04.30

                strSQL.Append(" LEFT OUTER JOIN ITEM_MASTER I ON SA.ITEM_NO              = I.ITEM_NO      ");
                strSQL.Append(" LEFT OUTER JOIN SYSTEM_PARAMETER S1 ON S1.KEY01          = 'ITEM_UMSR'    ");
                strSQL.Append(" AND S1.KEY02    =   I.ITEM_UMSR                                           ");
                strSQL.Append(" WHERE SH.ORDER_NO           = @ORDER_NO                                   ");
                strSQL.Append(" AND SH.ORDER_LINE_NO        = @ORDER_LINE_NO                              ");
                //strSQL.Append(" AND SA.SHIP_SEQ             = @SHIP_SEQ                                   ");
                strSQL.Append(" ORDER BY SA.SEQ_NO                              ");
