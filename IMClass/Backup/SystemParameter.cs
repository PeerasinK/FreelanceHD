...
namespace IMClass
{
    public class SystemParameter
    {
        ...
        public DataSet GetHeaderList()
        {
            ...
            strSQL.Append(" FROM SYSTEM_PARAMETER_HEADER ");
            /** IMV3 START **/
            strSQL.Append(" WHERE KEY01 NOT IN (");
            strSQL.Append(" 'ALLOC_CD','ALLOC_TYPE','BOM_EFF_DATE_TYPE','COM_PRC_FLAG','COST_CENTER','DISB_PRICE_STANDARD',");
            strSQL.Append(" 'IMIF_OPTION','ON_TIME_DELIVERY_PERCENT','PLAN_PERIOD','PRODPLAN_DECIMAL_DISP','RESOURCE_GRP','SCH_ID','SCH_ID_PASSWORD_CHECK','SDMIF_OPTION','TAX_CD','WS_PROC_TYPE'");
            strSQL.Append(" )");
            /** IMV3 END **/
        ...
        /// <summary>
        /// 
        /// USE_RMパラメータが1のとき、trueを返す
        /// 
        /// 使用画面：InvPickingInst, InvPickingInstByParent, InvStockInq
        /// </summary>
        public bool CheckUseRM()
        {
            ComDB db = new ComDB();
            StringBuilder strSQL = new StringBuilder();
            DataSet ds = new DataSet();
            ...
            //--- sql
            strSQL.Append(" SELECT DATA_INT ");

            strSQL.Append(" FROM SYSTEM_PARAMETER                               ");
            strSQL.Append(" WHERE   KEY01       =       'USE_RM'                ");

            try
            {
                ds = db.DbDataSet(strSQL.ToString(), "SYSPARA");
                if (ds == null)
                {
                    _dbmsg = db.expmsg;
                    _strErr = db.strErr;
                ...
                {
                    if (ds.Tables[0].Rows.Count > 0 && int.Parse(ds.Tables[0].Rows[0]["DATA_INT"].ToString()) == 1)
                        blRtn = true;
                }
                if (db.State() == ConnectionState.Open)
                    db.DbClose();
        ...
        /// <summary>
        /// 
        /// INPUT_MANDATORYパラメータが1のとき、trueを返す
        /// 
        /// 使用画面：HTProcessAct
        /// </summary>
        public bool CheckInputMandatory()
        {
            ComDB db = new ComDB();
            StringBuilder strSQL = new StringBuilder();
            DataSet ds = new DataSet();
            ...
            strSQL.Append(" FROM SYSTEM_PARAMETER                               ");
            strSQL.Append(" WHERE   KEY01       =       'INPUT_MANDATORY'                ");
