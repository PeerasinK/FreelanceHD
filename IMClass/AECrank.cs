...
namespace IMClass
{
    public class AECrank
    {
        ...
        public AECrank(string p_user_id, int p_lang)
        {
        ...
        public DataSet GetAECrankList()
        {
            ...
            strSQL.Append("C.[AE_Assy_Crank_No],    C.[Model],          C.[Part_No],        C.[Engine_No],          C.[Crank_Offline_Lot], ");
            strSQL.Append("C.[Create_Date],         C.[Create_By],      C.[Shift],          C.[Flag] ");

            if (_db_type == "ORACLE")
            {
                strSQL.Append(" FROM AE_ASSY_CRANK C ");
                strSQL.Append(" WHERE 1=1 ");
                if (_condition_DateFrom.HasValue) strSQL.Append(" AND CAST(C.Create_Date AS DATE) >= CAST('" + _condition_DateFrom.Value + "' AS DATE) ");
                if (_condition_DateTo.HasValue) strSQL.Append(" AND CAST(C.Create_Date AS DATE) <= CAST('" + _condition_DateTo.Value + "' AS DATE) ");
