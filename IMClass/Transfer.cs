...
namespace IMClass
{
    public class Transfer
    {
        ...
        public string die_no { get { return _die_no; } set { _die_no = value; } }
        ...
        /// </summary>
        /// 
        public DataSet GetUnplannedRcvActList2()
        {
            ComDB db = new ComDB(_db_type);
            StringBuilder strSQL = new StringBuilder();
            DataSet ds = new DataSet();
            ...
            //--- sql 
            strSQL.Append(" SELECT                                                              ");
            strSQL.Append(" T.RECEIVE_NO,T.PART_NO,T.LOT_NO,T.RECEIVE_QTY,                      ");
            strSQL.Append(" T.DIE_NO,T.SHIFT                                                    ");

            //db.DbParametersClear();

            if (_to_process_cd == "Crank Shaft")
            {
                strSQL.Append(" FROM REC_FE T                                                       ");
                strSQL.Append(" WHERE LOT_NO NOT IN (SELECT LOTNO_ZOZAI FROM PROD_CRANK_ONLINE_HEADER) ");
            }
            else
            {
                strSQL.Append(" FROM REC_FE2 T                                                       ");
                strSQL.Append(" WHERE LOT_NO NOT IN (SELECT LOTNO_ZOZAI FROM PROD_CONN_ONLINE_HEADER) ");
            }
        ...
        /// </summary>
        /// 
        //public DataSet GetUnplannedRcvActList3()
        //{
        //    ComDB db = new ComDB(_db_type);
        //    StringBuilder strSQL = new StringBuilder();
        //    DataSet ds = new DataSet();
        ...
        /// </summary>
        /// 
        public DataSet GetUnplannedDisbActList2()
        {
            ComDB db = new ComDB(_db_type);
            StringBuilder strSQL = new StringBuilder();
            DataSet ds = new DataSet();
            ...
            //--- sql 
            strSQL.Append(" SELECT                                                              ");
            strSQL.Append(" T.FE_CRANK_ONLINE_NO,T.PART_NO,T.LOTNO_ZOZAI,T.ONLINE_DATE,         ");
            strSQL.Append(" T.RECEIVE_QTY,T.DIE_NO,T.MODEL,D.SHIFT                              ");
            ...
            else
            {
                ...
                strSQL.Append(" LEFT JOIN PLC_CONN_LESER_MARK P ON T.LOTNO_ZOZAI = SUBSTRING(P.SERIALCODE, 22, 8)    ");
                strSQL.Append(" WHERE P.SERIALCODE IS NULL                                      ");
            }
        ...
        /// 使用画面：FeOnProd
        /// </summary>
        public int UnplannedRcvDisbAct2()
        {
            ComDB db = new ComDB(_db_type);
            int rtn = 0;
            ...
            db.DbPsetInt("RET", rtn, ComConst.DB_RTN);
            db.DbPsetString("@I_RECEIVE_NO", _slip_no, ComConst.DB_IN);
            //db.DbPsetInt("@I_ACT_DATE", _act_date, ComConst.DB_IN);
            ...
            db.DbBeginTrans();
            rtn = db.DbStored("SP_UNPLANNED_RCV_DISB_ACT2");
            rtn = db.DbPgetInt("RET");
            if (rtn == ComConst.SUCCEED)
            {
                db.DbCommit();
        ...
        /// 使用画面：FeOnProd
        /// </summary>
        public int UnplannedRcvDisbAct2A()
        {
            ComDB db = new ComDB(_db_type);
            int rtn = 0;
            ...
            db.DbBeginTrans();
            rtn = db.DbStored("SP_UNPLANNED_RCV_DISB_ACT2A");
            rtn = db.DbPgetInt("RET");
            if (rtn == ComConst.SUCCEED)
            {
                db.DbCommit();
        ...
        public int FEReceiveHT()
        {
            ...
            db.DbPsetString("@I_ITEM_NO", _item_no, ComConst.DB_IN);
            db.DbPsetString("@I_LOT_NO", _lot_no, ComConst.DB_IN);
            db.DbPsetDouble("@I_RECEIVE_QTY", _act_qty, ComConst.DB_IN);
            db.DbPsetString("@I_DIE_NO", _die_no, ComConst.DB_IN);
            db.DbPsetString("@I_SHIFT", _shift, ComConst.DB_IN);
            db.DbPsetString("@I_CHG_USER_ID", _chg_user_id, ComConst.DB_IN);
            db.DbPsetInt("@I_LANG", _lang, ComConst.DB_IN);
            db.DbPsetInt("@O_ERRCODE", _errcode, ComConst.DB_OUT);
            db.DbPsetString("@O_MSG", _errmsg, ComConst.DB_OUT);
        ...
        public int FEOffSwap()
        {
            ...
            db.DbBeginTrans();
            rtn = db.DbStored("SP_SWAP_OFF_FE");
            rtn = db.DbPgetInt("RET");
            if (rtn == ComConst.SUCCEED)
            {
                db.DbCommit();
        ...
        public int FEOffSwapA()
        {
            ...
            db.DbBeginTrans();
            rtn = db.DbStored("SP_SWAP_OFF_FEA");
            rtn = db.DbPgetInt("RET");
            if (rtn == ComConst.SUCCEED)
            {
                db.DbCommit();
