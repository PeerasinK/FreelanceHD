...
namespace IMClass
{
    public class Search
    {
        ...
        protected string _auto_foucus_control_id = string.Empty;
        public string auto_foucus_control_id
        {
            get { return _auto_foucus_control_id; }
            set { _auto_foucus_control_id = value; }
        }
        ...
        /// ロケーション検索画面
        /// </summary>
        public class SrchLocation
        {
            //ロケーション検索画面URL
            public const string URL = "../../Common/Search/SrchLocation.aspx";
            //呼出元に追加されるJavaScriptのFunction名

            //  画面内で重ならないようにして下さい：ただし同一の検索画面は画面内で連番をふりますので気にしないで下さい


            public const string FUNCTION_NAME = "ReturnSrchLocation";
            //画面の高さ
            public const int HEIGHT = 450;
            //画面の幅
        ...
        /// 製造指示検索画面
        /// </summary>
        public class SrchJobOrder
        {
            //製造指示検索画面URL
            public const string URL = "../../Common/Search/SrchJobOrder.aspx";
            //呼出元に追加されるJavaScriptのFunction名

            //  画面内で重ならないようにして下さい：ただし同一の検索画面は画面内で連番をふりますので気にしないで下さい


            public const string FUNCTION_NAME = "ReturnSrchJobOrder";
            //画面の高さ
            public const int HEIGHT = 550;
            //画面の幅
        ...
        /// 発注残検索画面  
        /// </summary>
        public class SrchPurOrder
        {
            //発注残検索画面URL
            public const string URL = "../../Common/Search/SrchPurOrder.aspx";
            //呼出元に追加されるJavaScriptのFunction名

            //  画面内で重ならないようにして下さい：ただし同一の検索画面は画面内で連番をふりますので気にしないで下さい


            public const string FUNCTION_NAME = "ReturnSrchPurOrder";
            //画面の高さ
            public const int HEIGHT = 550;
            //画面の幅
        ...
        public int CreateSearchWindow()
        {
            ...
            string strSearchUrl = ""; string strReturnFunctionName = ""; int intHeight = 0; int intWidth = 0;
            rtn = GetSerchFormInfo(out strSearchUrl, out strReturnFunctionName, out intHeight, out intWidth);
            ...
            {               
                ...
                if (obj != null)
                {
                    ...
                    if (obj.GetType().Name == "Label")
                        ...
                        strReturnScript.Append("value=elemValue[" + i + "];\n");
                    if (_auto_foucus_control_id != string.Empty)
                    {
                        if (i == 0)
                        {
                            strReturnScript.Append("  document.forms[0].elements['" + _auto_foucus_control_id + "'].select();\n");
                        }
                    }
                    else
                    {
            ...
            // すでに表示済の場合は、いったん閉じる 2015.06.15
            //strOpenScript.Append("var w=window.open('','" + _open_window_name + "','left=3000,top=3000'); w.close();");
            strOpenScript.Append("var w=window.open('','" + _open_window_name + "','left=3000,top=3000');\n if(w!=null)\n w.close();\n"); //2015.10.28 upd

            strOpenScript.Append("var w2;\n");                      //2015.10.30 add
            strOpenScript.Append("for (var i=1; i<=10; i++){\n");   //2015.10.30 add
            strOpenScript.Append("try{\n");                         //2015.10.30 add

            //strOpenScript.Append("window.open('");
            strOpenScript.Append("w2=window.open('");   //2015.10.30 upd
            //strOpenScript.Append(strSearchUrl).Append("?ScriptName=").Append(strReturnFunctionName).Append("', ");
            //strOpenScript.Append(strSearchUrl).Append("?ScriptName=").Append(strReturnFunctionName).Append("&WS_CD=" + ws_cd + "', ");//ADD BY LIU 2009.1.8       //Del 13.08.22 Ubiq-Suo
            strOpenScript.Append(strSearchUrl).Append("?ScriptName=").Append(strReturnFunctionName).Append("&WS_CD='+encodeURIComponent('" + ws_cd + "'), ");       //Add 13.08.22 Ubiq-Suo
            //strOpenScript.Append("null, 'left=400, top=100, status=no, toolbar= no, location= no, menubar= no ");
            strOpenScript.Append("'" + _open_window_name + "', 'left=400, top=100, status=no, toolbar= no, location= no, menubar= no ");
            strOpenScript.Append(",height=" + intHeight.ToString());
            strOpenScript.Append(",width=" + intWidth.ToString());
            strOpenScript.Append("');\n");
            strOpenScript.Append("return false;");      //2015.10.30 add

            strOpenScript.Append("\n} catch(e){");    //2015.10.30 add
            strOpenScript.Append("\n if (i>=10) window.alert(e.message);");     //DEBUG
            strOpenScript.Append("\n}");                //2015.10.30 add
            strOpenScript.Append("\n}\n");              //2015.10.30 add
            strOpenScript.Append("return false;");      //2015.10.30 add

            // ボタンへイベントを登録
            _opener_button.Attributes.Add("onclick", strOpenScript.ToString());
        ...
        /// <returns></returns>
        private int GetSerchFormInfo(out string p_searchUrl, out string p_returnFunctionName, out int p_height, out int p_width)
        {
            int cnt = 0;
            p_searchUrl = "";
            ...
            switch (_search_type)
            {
                ...
                // IMV3
                case SearchType.LOCATION: //ロケーション検索画面
                    p_searchUrl = SrchLocation.URL;
                    p_returnFunctionName = SrchLocation.FUNCTION_NAME + "_0";
                    cnt = 0;
                    while (clientScriptManager.IsClientScriptBlockRegistered(_opener.GetType(), p_returnFunctionName))
                    {
                        cnt++;
                        p_returnFunctionName = SrchLocation.FUNCTION_NAME + "_" + cnt.ToString();
                    }
                    p_height = SrchLocation.HEIGHT;
                    p_width = SrchLocation.WIDTH;
                    break;
                // IMV3
                case SearchType.JOB_ORDER: //製造指示検索画面
                    p_searchUrl = SrchJobOrder.URL;
                    p_returnFunctionName = SrchJobOrder.FUNCTION_NAME + "_0";
                    cnt = 0;
                    while (clientScriptManager.IsClientScriptBlockRegistered(_opener.GetType(), p_returnFunctionName))
                    {
                        cnt++;
                        p_returnFunctionName = SrchJobOrder.FUNCTION_NAME + "_" + cnt.ToString();
                    }
                    p_height = SrchJobOrder.HEIGHT;
                    p_width = SrchJobOrder.WIDTH;
                    break;
                // IMV3
                case SearchType.PUR_ORDER: //発注残検索画面
                    p_searchUrl = SrchPurOrder.URL;
                    p_returnFunctionName = SrchPurOrder.FUNCTION_NAME + "_0";
                    cnt = 0;
                    while (clientScriptManager.IsClientScriptBlockRegistered(_opener.GetType(), p_returnFunctionName))
                    {
                        cnt++;
                        p_returnFunctionName = SrchPurOrder.FUNCTION_NAME + "_" + cnt.ToString();
                    }
                    p_height = SrchPurOrder.HEIGHT;
                    p_width = SrchPurOrder.WIDTH;
                    break;
