...
{
    public partial class ImpPlanProdOrd : System.Web.UI.Page
    {
            ...
            switch (ddlTYPE.Text.ToString())
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                        ...
                        rtn = 0;
                        mc1.OptMethod = "MC";
                        rtn = mc1.GetHeadNo(out rtnHEADNO);
                        ...
                        mc1.mc_prod_no = rtnHEADNO;
                        mc1.year = Convert.ToInt32(Convert.ToDateTime(dt.Rows[i]["Plan_Date"]).Year);
                        mc1.month = Convert.ToInt32(Convert.ToDateTime(dt.Rows[i]["Plan_Date"]).Month);
                        mc1.version = 0;
                        mc1.part_no = dt.Rows[i]["Part_No"].ToString();
                        mc1.remark = dt.Rows[i]["Remark"].ToString();
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        fe1.fe_prod_no = rtnHEADNO;
                        fe1.year = Convert.ToInt32(Convert.ToDateTime(dt.Rows[i]["Plan_Date"]).Year);
                        fe1.month = Convert.ToInt32(Convert.ToDateTime(dt.Rows[i]["Plan_Date"]).Month);
                        fe1.version = 0;
                        fe1.part_no = dt.Rows[i]["Part_No"].ToString();
                        fe1.remark = dt.Rows[i]["Remark"].ToString();
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        sp1.server_part_no = rtnHEADNO;
                        sp1.year = Convert.ToInt32(Convert.ToDateTime(dt.Rows[i]["Plan_Date"]).Year);
                        sp1.month = Convert.ToInt32(Convert.ToDateTime(dt.Rows[i]["Plan_Date"]).Month);
                        sp1.version = 0;
                        sp1.part_no = dt.Rows[i]["Part_No"].ToString();
                        if (comlb.IsNumeric(dt.Rows[i]["SEMI_FIXED_FIRST"].ToString()))
                        {
                            sp1.semi_fixed_first = Convert.ToInt32(dt.Rows[i]["SEMI_FIXED_FIRST"].ToString());
                        }
                        else
                        {
                            sp1.semi_fixed_first = 0;
                        }
                        if (comlb.IsNumeric(dt.Rows[i]["SEMI_FIXED_SECOND"].ToString()))
                        {
                            sp1.semi_fixed_second = Convert.ToInt32(dt.Rows[i]["SEMI_FIXED_SECOND"].ToString());
                        }
                        else
                        {
                            sp1.semi_fixed_second = 0;
                        }
                        //sp1.semi_fixed_first = Convert.ToInt32(dt.Rows[i]["SEMI_FIXED_FIRST"].ToString());
                        //sp1.semi_fixed_second = Convert.ToInt32(dt.Rows[i]["SEMI_FIXED_SECOND"].ToString());
