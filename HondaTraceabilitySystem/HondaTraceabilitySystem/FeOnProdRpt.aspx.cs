...
{
    public partial class FeOnProdRpt : System.Web.UI.Page
    {
        ...
        protected void cmdPrint_Click(object sender, EventArgs e)
        {
            ...
            prtDt.Columns.Add(new DataColumn("RECEIVE_QTY"));
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (com.IntToBool(com.StringToInt(dt.Rows[i]["SELECT"].ToString())))
                {
                    ...
                    if (seqno == 0)
                    {
                        h_period_from = "PERIOD FROM : " + dt.Rows[i]["ONLINE_DATE"].ToString();
                    }
                    seqno++;

                    DataRow dr = prtDt.NewRow();
                    dr["FE_CRANK_ONLINE_NO"] = dt.Rows[i]["ONLINE_NO"].ToString();
                    dr["PRODUCTION_DATE"] = dt.Rows[i]["ONLINE_DATE"];
                    dr["SHIFT"] = dt.Rows[i]["SHIFT"].ToString();
                    dr["MODEL"] = dt.Rows[i]["MODEL"].ToString();
                    dr["LOTNO_ZOZAI"] = dt.Rows[i]["LOTNO_ZOZAI"].ToString();
                    dr["DIE_NO"] = dt.Rows[i]["DIE_NO"].ToString();
                    dr["RECEIVE_QTY"] = dt.Rows[i]["RECEIVE_QTY"];
                    dr["ONLINE_STAMPING"] = dt.Rows[i]["ONLINE_STAMPING"].ToString();
                    dr["REMARK"] = dt.Rows[i]["REMARK"].ToString();

                    prtDt.Rows.Add(dr);

                    h_period_to = "           TO : " + dt.Rows[i]["ONLINE_DATE"].ToString();
                }
            }
            if (print_flag == false)
            {
            ...
            try
            {
                ...
                Rpt.SetParameterValue("H_DATA_TYPE", "DATA TYPE : ON LINE");
                Rpt.SetParameterValue("H_TYPE", "TYPE :");
                Rpt.SetParameterValue("H_DEPT", "DEPT. : FE-P");
                if (ddlPROCESS.SelectedValue == "Crank Shaft")
                {
                    Rpt.SetParameterValue("H_PART_NAME", "PART NAME : CRANK ZOZAI");
                    Rpt.SetParameterValue("H_LINE", "LINE : CRANK LINE");
                }
                else
                {
                    Rpt.SetParameterValue("H_PART_NAME", "PART NAME : CONNROD ZOZAI");
