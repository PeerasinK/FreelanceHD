...
{
    public partial class SrchUser : System.Web.UI.Page
    {
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    switch (row["CONTROL_ID"].ToString())
                    {
                        ...
                        case "flblCount": flblCount.Text = "[" + row["ITEM_DESC"].ToString() + " = "; break;
                        ...
                        // 一覧スプレッド
                        case "dgvUSER_ID": this.dgvSearch.Columns[2].HeaderText = row["ITEM_DESC"].ToString(); break;
                        case "dgvUSER_DESC": this.dgvSearch.Columns[3].HeaderText = row["ITEM_DESC"].ToString(); break;
                    }
                }
            }
            //
        ...
        protected void Edit_Grid()
        {
            ...
            DataSet ds = us.GetUserSearchList();
        ...
        protected void Script_Proc()
        {
            ...
            sb.Append(@"    $('#hdnUserID').val(item1);   ");
            sb.Append(@"    $('#hdnUserName').val(item2);   ");
            sb.Append(@"    $('#basicModalUser').modal('hide');   ");
            //sb.Append(@"    $('#basicModalUser2').modal('hide');   ");
            sb.Append(@"} ");
            //sb.Append(@"    var arr0 = item1;");   //ユーザーID
            //sb.Append(@"    var arr1 = item2;");   //ユーザー名称   
        ...
        protected void dgvSearch_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.DataItemIndex >= 0)
            {
                ...
                strEvent = String.Format("SelectedRow('{0}', '{1}')", com.SetJsStr(((TextBox)e.Row.FindControl("lblUserID")).Text), com.SetJsStr(((TextBox)e.Row.FindControl("lblUserName")).Text));
