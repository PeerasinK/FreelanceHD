...
{
    public partial class SrchLocation : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //   i_ws_cd = Request["WS_CD"].ToString();
            //i_ws_cd = "WS001";
            //}
            //catch
            //{
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    switch (row["CONTROL_ID"].ToString())
                    {
                        ...
                        // 一覧スプレッド
                        case "dgvLOCATION": this.dgvSearch.Columns[1].HeaderText = row["ITEM_DESC"].ToString(); break;
                        case "dgvLOCATION_DESC": this.dgvSearch.Columns[2].HeaderText = row["ITEM_DESC"].ToString(); break;
                    }
                }
            }
            //
        ...
        protected void Script_Proc()
        {
            ...
            sb.Append(@"    $('#hdnLocNo').val(item1);   ");
            sb.Append(@"    $('#hdnLocNoDesc').val(item2);   ");
            sb.Append(@"    $('#basicModalLoc').modal('hide');   ");
            //sb.Append(@"    $('#basicModalLoc2').modal('hide');   ");
            sb.Append(@"} ");
        ...
        protected void dgvSearch_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.DataItemIndex >= 0)
            {
                ...
                strEvent = String.Format("SelectedRow('{0}', '{1}')", com.SetJsStr(((TextBox)e.Row.FindControl("lblLocCd")).Text), com.SetJsStr(((TextBox)e.Row.FindControl("lblLocDesc")).Text));
