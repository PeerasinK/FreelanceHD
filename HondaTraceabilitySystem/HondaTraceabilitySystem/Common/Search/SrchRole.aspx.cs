...
{
    public partial class SrchRole : System.Web.UI.Page
    {
        ...
        protected void Script_Proc()
        {
            ...
            sb.Append(@"    $('#hdnUG').val(item1);   ");
            sb.Append(@"    $('#hdnUGDesc').val(item2);   ");
            sb.Append(@"    $('#basicModalUG').modal('hide');   ");
            //sb.Append(@"    $('#basicModalUG2').modal('hide');   ");
            sb.Append(@"} ");
        ...
        protected void dgvSearch_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.DataItemIndex >= 0)
            {
                ...
                strEvent = String.Format("SelectedRow('{0}', '{1}')", com.SetJsStr(((TextBox)e.Row.FindControl("lblUG")).Text), com.SetJsStr(((TextBox)e.Row.FindControl("lblUGDesc")).Text));
