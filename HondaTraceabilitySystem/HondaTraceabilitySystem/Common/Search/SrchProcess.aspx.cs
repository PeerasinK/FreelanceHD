...
{
    public partial class SrchProcess : System.Web.UI.Page
    {
            ...
            if (ds != null && ds.Tables.Count > 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    switch (row["CONTROL_ID"].ToString())
                    {
                        ...
                        // ボタン
                        //case "btnSelect": btnSelect.Text = row["ITEM_DESC"].ToString(); break;
                        //case "btnClose": btnClose.Text = row["ITEM_DESC"].ToString(); break;

                        // 一覧スプレッド
                        case "dgvPROCESS_CD": this.dgvSearch.Columns[1].HeaderText = row["ITEM_DESC"].ToString(); break;
                        case "dgvPROCESS_DESC": this.dgvSearch.Columns[2].HeaderText = row["ITEM_DESC"].ToString(); break;
                    }
                }
            }
            //
        ...
        protected void Edit_Grid()
        {
            ...
            //SrchProcess.process_cd = txtProcess.Text.ToUpper();
        ...
        protected void Script_Proc()
        {
            ...
            sb.Append(@"    $('#hdnProcessNo').val(item1);   ");
            sb.Append(@"    $('#hdnProcessNoDesc').val(item2);   ");
            sb.Append(@"    $('#basicModalProcess').modal('hide');   ");
            //sb.Append(@"    $('#basicModalProcess2').modal('hide');   ");
            sb.Append(@"} ");
        ...
        protected void dgvSearch_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.DataItemIndex >= 0)
            {
                ...
                strEvent = String.Format("SelectedRow('{0}', '{1}')", com.SetJsStr(((TextBox)e.Row.FindControl("lblProcessCd")).Text), com.SetJsStr(((TextBox)e.Row.FindControl("lblProcessDesc")).Text));
